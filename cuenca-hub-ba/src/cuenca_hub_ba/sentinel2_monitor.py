...
class Sentinel2ResearchGrade:
    ...
    def _get_research_grade_image(self):
        ...
        if not items:
            raise ValueError(
                f"No se encontraron imágenes Sentinel-2 para el área y fechas especificadas. Rango: {self.date_range}"
            )
        ...
        print(
            f"   ☁️ Cobertura de nubes: {selected.properties.get('eo:cloud_cover', 'N/A')}%"
        )
    ...
    def _load_and_resample(self, assets, reference_band="green"):
        ...
        def read_band(href, band_name):
            with rasterio.open(href) as src:
                ...
                with WarpedVRT(
                    src, **master_profile, resampling=Resampling.bilinear
                ) as vrt:
                    # Normalización a Reflectancia de Superficie (0-1)
                    # Sentinel-2 L2A viene en enteros (0-10000)
                    return (
                        vrt.read(1, out_shape=target_shape).astype("float32") / 10000.0
                    )
        ...
        try:
            data = {
                ...
                "B5_RedEdge": read_band(
                    assets["rededge1"].href, "RedEdge1"
                ),  # Nativa 20m -> 10m
                "B8_NIR": read_band(assets["nir"].href, "NIR"),
                "B11_SWIR": read_band(
                    assets["swir16"].href, "SWIR1"
                ),  # Nativa 20m -> 10m
            }
            ...
            with rasterio.open(assets["scl"].href) as src:
                with WarpedVRT(
                    src, **master_profile, resampling=Resampling.nearest
                ) as vrt:
    ...
    def analyze(self):
        ...
        mndwi = (bands["B3_Green"] - bands["B11_SWIR"]) / (
            bands["B3_Green"] + bands["B11_SWIR"]
        )
        ...
        if valid_pixels_count == 0:
            return {
                "error": "AOI sin píxeles de agua válidos (posible obstrucción de nubes)."
            }
        ...
        ndci = (bands["B5_RedEdge"] - bands["B4_Red"]) / (
            bands["B5_RedEdge"] + bands["B4_Red"]
        )
        ...
        fai = bands["B8_NIR"] - (
            bands["B4_Red"] + (bands["B11_SWIR"] - bands["B4_Red"]) * 0.5
        )

        # --- NDTI (Normalized Difference Turbidity Index) ---
        # Ref: Lacaux et al. (2007). Correlaciona con sólidos suspendidos totales (TSS).
        ndti = (bands["B4_Red"] - bands["B3_Green"]) / (
            bands["B4_Red"] + bands["B3_Green"]
        )
        ...
        floating_area_ha = (
            floating_veg_pixels * self.PIXEL_AREA_M2 * self.SQM_TO_HECTARES
        )
            ...
            "indicators": {
                "eutrophication_ndci": {
                    ...
                    "classification_breakdown_ha": {
                        "clean_oligotrophic": round(
                            oligotrophic * self.PIXEL_AREA_M2 * self.SQM_TO_HECTARES, 2
                        ),
                        "moderate_mesotrophic": round(
                            mesotrophic * self.PIXEL_AREA_M2 * self.SQM_TO_HECTARES, 2
                        ),
                        "high_eutrophic": round(
                            eutrophic * self.PIXEL_AREA_M2 * self.SQM_TO_HECTARES, 2
                        ),
                        "critical_hypertrophic": round(
                            hypertrophic * self.PIXEL_AREA_M2 * self.SQM_TO_HECTARES, 2
                        ),
                    },
                },
                "macrophytes_fai": {
                    ...
                    "percentage_coverage": round(
                        (floating_area_ha / total_area_ha) * 100, 2
                    ),
                    "invasion_status": "CRITICAL"
                    if (floating_area_ha / total_area_ha) > 0.15
                    else "MONITOR",
                },
                "turbidity_ndti": {
                    "mean_value": round(float(np.mean(data_ndti)), 4),
                    "sediment_load_status": "HIGH"
                    if np.mean(data_ndti) > 0.1
                    else "NORMAL",
                },
                "cyanobacteria_risk": {
                    "mean_ratio_2bda": round(float(np.mean(data_cyano)), 4),
                    "high_risk_area_ha": round(
                        toxic_risk_pixels * self.PIXEL_AREA_M2 * self.SQM_TO_HECTARES, 2
                    ),
                },
            },
            "quality_control": {
                "cloud_probability_percent": item.properties.get(
                    "s2:cloud_probability", 0
                ),
